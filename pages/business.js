...
export default function business({data}) {
    ...
    return (
        <>
        <div>
            <h1 className="heading">Top Business Headlines!</h1>
        </div>
...
export async function getStaticProps() {
    const response = await fetch(`https://newsapi.org/v2/top-headlines?category=business&country=in&apiKey=${process.env.NEWS_API_KEY}`)
